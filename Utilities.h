...
namespace Sel {
  ...
  template <typename particle_t>
    ...
    auto const mom   = threeMomentum( deref_if_ptr( p ) );
    auto const pos   = referencePoint( deref_if_ptr( p ) );
    ...
    s.covXX() = LHCb::LinAlg::similarity( Jxpos, posCovMatrix( deref_if_ptr( p ) ) );
    s.covTT() = LHCb::LinAlg::similarity( Jtxmom, threeMomCovMatrix( deref_if_ptr( p ) ) );
	...
	auto const cov44 = LHCb::LinAlg::similarity( J, cov66 );
